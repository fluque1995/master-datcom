...
En el apartado anterior sólo hemos utilizado las variables numéricas
de las que disponíamos, despreciando la información almacenada en las
variables categóricas. En este apartado estudiaremos cómo podemos
incorporar dicha información en el cómputo de los _clusters_. De
nuevo, vamos a intentar acomodar los grupos calculados a la clase que
se nos proporciona en el problema. Usaremos por tanto la variable
educación como variable categórica, la cual convertiremos a un
conjunto de variables binarias (variables _dummy_) sobre las que
calcularemos una distancia binaria. Después, combinaremos esta
distancia con la calculada sobre las variables continuas para definir
la distancia final.
...
final.dist <- (scaled.dist * length(scaled.data) + dummy.dist * length(dummy.data)) /
    (length(scaled.data) + length(dummy.data))

kmeans.res <- kmeans(final.dist, nclust)

# autoplot(kmeans.res, data = scaled.data)
```

Una vez estudiado el algoritmo de las k-medias, pasamos a otro algoritmo
de particionado por distancias, conocido como DBSCAN.
...
fviz_cluster(dbscan.res, data = scaled.data, outlier.pointsize = .1, geom = "point")
```
...
centros de los clusters sean puntos del conjunto de datos produce
unos resultados significativamente distintos
