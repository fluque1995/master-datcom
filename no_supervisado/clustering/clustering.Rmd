...
Podemos observar que los resultados obtenidos no son especialmente
buenos a la hora de clasificar los elementos de nuestro conjunto de
datos en pagadores y no pagadores. En efecto, si mostramos el mismo
gráfico que anteriormente, pero utilizando la columna impago como
etiqueta, obtenemos el siguiente resultado:
...
```{r}
scaled.distances <- dist(scaled.data) # Calculamos las distancias
...
numeric.vars <- c('ingresos', 'deudaingr', 'deudacred', 'deudaotro',
                  'empleo',  'direccion', 'edad')
nominal.vars <- c('educ')
...
la modificación en la distancia. Esta problemática puede venir
justificada por diversos factores. Es posible que la información
codificada en las variables nominales suponga un ruido en nuestro
problema, y sea más adecuado prescindir de ellas. Otra posibilidad
es que la forma de combinar las distancias que hemos elegido no sea
óptima. En el ejemplo anterior, utilizamos una media aritmética
entre las dos matrices. Realmente, en nuestro problema, la matriz
de distancias correspondiente a las variables numéricas debería
tener, a priori, un mayor peso, ya que el número de variables
numéricas de las que se dispone es significativamente mayor al
número de variables nominales. Si no se ponderan las matrices de
distancia en consecuencia, se puede estar introduciendo un sesgo
indeseado en el cálculo de las distancias.

Otra forma de afrontar la combinación de las distancias viene motivada
por la importancia que queramos dar a las variables. Podríamos querer
dar más importancia a la información que contienen las variables
nominales que las variables categóricas. Por ejemplo, si en lugar
de hacer la media aritmética, hacemos una media ponderada en la
que damos tres veces más peso a las distancias nominales que a las
numéricas, el resultado que obtenemos es el siguiente:
...
Donde podemos observar, ahora sí, una diferencia sustancial en el
resultado obtenido. Si medimos sobre este nuevo agrupamiento el
coeficiente de silueta, obtenemos el siguiente resultado:
...
Donde obtenemos un agrupamiento de mejor calidad que en el caso
anterior. A pesar de ello, aún quedan algunos puntos en el primero de
los clusters con un coeficiente de silueta negativo, pero la cantidad
de elementos en los que ocurre esto es significativamente menor que
en el ejemplo anterior.

Vamos a comprobar ahora si con el nuevo agrupamiento que hemos
conseguido, se consigue discriminar entre los clientes que han
incurrido en impago. Cuando ejecutamos el algoritmo de las k-medias
vimos que el algoritmo no era capaz de separar correctamente estos
dos grupos. En el agrupamiento que hemos generado ahora:

```{r echo=F}
conf.mat <- table(pam.result$cluster, dataset$impago)
rownames(conf.mat) <- c(0, 1)
kable(conf.mat)
```

Seguimos sin poder discriminar completamente entre estas dos clases,
pero la cantidad de ejemplos bien clasificados ha aumentado
notablemente. El número de ejemplos bien clasificados en el cluster de
gente que no ha incurrido en impago ha aumentado en varios elementos,
y especialmente para el otro cluster, el número de ejemplos bien
clasificados se ha duplicado, prácticamente. Este hecho nos indica que
la variable nominal de la que disponemos, el nivel de estudios del
individuo, es una variable relevante a la hora de afrontar este
problema, y nos permite agrupar bien (al menos, mejor que las otras
variables con las que habíamos trabajado hasta el momento) los elementos
de nuestro conjunto. A pesar de ello, la solución aún dista mucho
de ser de calidad.

En el siguiente apartado, veremos una modificación de este algoritmo
que es capaz de inferir el número apropiado de agrupamientos en los
que dividir los datos, sin necesidad de ser establecido a priori.

## Búsqueda del valor óptimo de k

Como hemos dicho anteriormente, este método dispone de una extensión
que le permite estimar el número de clusters en los que debe dividirse
el conjunto de datos. Este método ejecuta el algoritmo utilizando el
número de clusters en un rango especificado y selecciona el resultado
más adecuado en función de un criterio especificado de antemano.

En nuestro caso utilizaremos como criterio el coeficiente de silueta,
que es la métrica principal con la que estamos evaluando nuestros
resultados, y es la métrica que nuestro método utiliza por defecto.
El rango del número de grupos que consideraremos será entre 2 y 10
clusters. Excluimos el 1 porque no nos interesa agrupar todos los
puntos dentro del mismo grupo. No obstante, existe la posibilidad de
agrupar todos los puntos en el mismo grupo, y existen medidas que nos
permiten evaluar la calidad de dicho agrupamiento frente a
agrupamientos con más clusters (trivialmente, el coeficiente de
silueta carece de sentido cuando sólo se dispone de un cluster, ya que
es necesario calcular las distancias a puntos en otros clusters). El
límite superior se establece en 10 porque con un conjunto de datos tan
pequeño, un número mayor de clusters empieza a perder sentido, debido
a que el número de puntos que se asignan a cada cluster es muy
bajo. No obstante, si el número de clusters óptimo se situase en el
máximo que hemos escogido, sería recomendable aumentar dicho número y
repetir los experimentos.

Ejecutaremos el algoritmo sobre la matriz de distancias del apartado
anterior, que es la que mejores resultados nos ha proporcionado hasta
el momento. Mostramos la ejecución del algoritmo:
...
scaled.distances <- dist(scaled.data)
nominal.distances <- dist(dummy.data, method="binary")

final.distances <- 0.25*scaled.distances + 0.75*nominal.distances

pamk.result <- pamk(final.distances)
pamk.result$pamobject$data <- as.data.frame(scaled.data)

autoplot(pamk.result$pamobject, frame=T)
```

Utilizando el criterio del coeficiente de silueta, se encuentra que el
número óptimo de grupos para esta configuración de distancias es de 3.
Si observamos los coeficientes de silueta de este método, obtenemos:

```{r, echo=F}
sil <- silhouette(pamk.result$pamobject$cluster, final.distances)
plot(sil, col=1:3, main="Silhouette plot")
```

En efecto, podemos comprobar que los resultados obtenidos son mejores
que la agrupación en dos conjuntos que realizamos anteriormente,
habiéndose aumentado significativamente el coeficiente de silueta de
la configuración completa. A pesar de ello, aún existen algunos puntos
con un coeficiente de silueta negativo en los clusters 1 y 2.

Una vez hemos visto los métodos basados en prototipos, pasamos a ver
un método que afronta el problema del agrupamiento de una forma
completamente diferente, agrupando los puntos por densidad.

# DBSCAN

El algoritmo de agrupamiento DBSCAN es un algoritmo de agrupamiento
por distancias que trata de formar grupos de puntos por densidad. El
funcionamiento del algoritmo es el siguiente. Mientras no se hayan
agrupado todos los puntos:
...
Finalmente, pasamos a ver el último algoritmo que vamos a estudiar.
Se trata de una modificación del algoritmo de las k-medias, la cual
trabaja de forma difusa. Esto es, en lugar de asignar un punto a un
determinado grupo de forma única, lo que se asigna a cada punto es
una medida de pertenencia a cada cluster.

# Fuzzy k-means

Como hemos dicho anteriormente, ahora vamos a estudiar un algoritmo de
agrupamiento difuso, conocido como _fuzzy k-means_. Dicho algoritmo es
similar al algoritmo de las k-medias, pero en lugar de asignar cada
punto a un único cluster, se tiene una función de similaridad, la cual
asigna a cada punto una medida de pertenencia del mismo a todos los
clusters que se tienen. De esta manera. la información que se obtiene
sobre los ejemplos del conjunto de datos es más rica.

Algorítmicamente, la solución a este problema se resuelve de la
siguiente manera. Dado un conjunto de puntos $X = \{x_1, ..., x_n\}$,
y un valor $k \in \mathbb{N}$, tenemos que separar los puntos en $k$
conjuntos difusos en función de una medida de distancia. Al igual
que en el algoritmo de las k-medias, trataremos de buscar $k$ puntos
del espacio de características $C = \{C_1, ..., C_k\}$ que representan
los centroides de los clusters, y una matriz $W = [0,1]^{n \times k}$,
tal que el valor $w_{ij}$ de la matriz representa el índice de pertenencia
del punto $x_i$ al cluster $j$.

El funcionamiento del algoritmo es el que sigue:

1. Se inicializan aleatoriamente los $k$ centroides.
2. Mientas no se cumpla la condición de parada (usualmente, mientras
   no se converja a un punto de equilibrio):
   - Se usan los centroides para calcular el grado de pertenencia de
   cada punto a cada cluster (se recalcula la matriz $W$ en función de
   la distancia de cada punto a cada centroide).
   - Con las nuevas medidas de pertenencia calculadas, se recalculan
   los centroides, haciendo una media ponderada de todos los puntos
   por el grado de pertenencia de los mismos al cluster en cuestión
   (matemáticamente, $c_j = \frac{\sum_{i=1}^n w^q_{ij}
   x_i}{w^q_{ij}}$).  El exponente $q$ es un parámetro del algoritmo
   que marca lo difuso que se permite que sea el cluster.

Cuando el algoritmo converge (los centroides y la matriz de pertenencia
son invariantes entre una iteración y la siguiente) el algoritmo termina.
Usualmente se especifica además un máximo de iteraciones como criterio
de parada, para evitar que el algoritmo no termine.

En el algoritmo anterior no hemos hecho hincapié en la forma de
calcular la pertenencia de los elementos a los clusters (el método de
cálculo de los valores $w_{ij}$). Existen distintas políticas para el
cálculo de la pertenencia. No obstante, la más utilizada suele ser un
decaimiento exponencial en función de la distancia del punto a cada
centroide, normalizado a posteriori para que la suma total de las
pertenencias a todos los clusters sea igual a 1 (de esta forma es más
fácil intepretar y comparar los resultados para los distintos puntos).
En este punto hay que definir la distancia utilizada, que en nuestro
caso será la distancia euclídea, al igual que hemos hecho a lo largo
del resto de la práctica.

En cuanto al valor del parámetro $q$, cuanto mayor sea dicho parámetro
más difusos serán los clusters resultantes. En particular, cuando $q =
1$, los clusters que se forman son clusters en el sentido clásico, con
grados de pertenencia 0 o 1. Usualmente, si no se tiene más
información, se suele inicializar al valor $q = 2$, y será el valor
que tomaremos nosotros por defecto.

Este algoritmo vuelve a presentar la problemática que teníamos en el
algoritmo de las k-medias, y que conseguimos arreglar con los modelos
que utilizaban los puntos del conjunto en lugar de prototipos
artificiales. Para este método volvemos a no poder combinar distancias
binarias y numéricas, así que trabajaremos exclusivamente con los
valores numéricos de los que disponemos.

## Aplicación del algoritmo

Pasamos a ver cómo aplicar el algoritmo a nuestro conjunto de
datos. De nuevo, comenzamos dividiendo el conjunto en dos clusters,
para intentar diferenciar entre las dos clases que tenemos:
...
fuzzy.result <- fanny(scaled.data, nclust, memb.exp=2)
```

Vemos que se nos genera un aviso que nos dice que los coeficientes de
pertenencia están todos muy cercanos a $1/k$, donde $k$ es el número
de clusters. Esto indica que con el valor $q = 2$, las distancias
entre los puntos no son suficientemente grandes para discriminar los
puntos.  En efecto, si miramos la matriz de pertenencia (imprimimos
sólo las tres primeras filas):

```{r echo=F}
kable(fuzzy.result$membership[1:3,],
      row.names = T,
      col.names = c("Cluster 1", "Cluster 2")
      )
```

Podemos observar que los valores de pertenencia calculados son
prácticamente iguales a $0.5$ en todos los casos. Se puede arreglar
esta problemática reduciendo el valor $q$. En efecto, variando el valor
en pasos de $0.1$, no conseguimos que el algoritmo converja hasta que
$q = 1.5$. Tomamos por tanto ese valor de $q$ como referencia. Los
resultados obtenidos para dicho valor son los siguientes:

```{r, fig.height=4}
fuzzy.result <- fanny(scaled.data, nclust, memb.exp = 1.5)
autoplot(fuzzy.result)
```

Y la matriz anterior se convierte en:
...
En la que los coeficientes de pertenencia sí se han desviado del
centro, lo que nos permite asignar con cierta confianza un cluster a
cada punto.

De nuevo, podemos estudiar la calidad del agrupamiento a través del
coeficiente de silueta:

```{r}
sil <- silhouette(fuzzy.result$cluster, scaled.distances)
plot(sil, col = 1:nclust)
```

Podemos observar que los resultados obtenidos no son especialmente
buenos, ya que en este caso nos va a ocurrir los mismo que nos ocurría
en el caso de las k-medias. Este conjunto de datos no es
particularmente adecuado para el uso de estos algoritmos, debido a que
la información más discriminatoria entre puntos está almacenada en la
variable nominal que mide el nivel educativo, la cual no es fácilmente
aprovechable en este tipo de algoritmos. Podríamos tratarla como una
variable numérica continua más, pero se podría introducir un sesgo
no deseado.

A modo de conclusión, vamos a poner un ejemplo de conjunto de datos en
el que los algoritmos de agrupamiento convexo dan buenos resultados, y
los clusters que se encuentran tienen una correspondencia con las
clases en las que hay que clasificar los elementos del conjunto.

# Anexo - Dataset optdigits

En este apartado, vamos a mostrar un ejemplo de dataset en el que los
algoritmos de clustering sí tienen un buen comportamiento. Dicho
conjunto de datos es conocido como optdigits. Dicho conjunto de datos
aparece tras un preprocesamiento sobre el conjunto de datos
MNIST. MNIST es un dataset compuesto por imágenes de tamaño $32 \times
32$ de dígitos manuscritos del 0 al 9. Optdigits se forma dividiendo
las imágenes anteriores en 64 parches de $4 \times 4$, y contando el
número de píxeles negros de cada parche (las imágenes están en blanco
y negro). De esta forma, tenemos para cada ejemplo 64 variables con
valores entre 0 y 16. Si aplicamos TSNE (un algoritmo de reducción de
dimensionalidad basado en vecindad) sobre los datos para
representarlos en un gráfico 2D, obtenemos el siguiente gráfico:

```{r}
optdigits <- read.csv("dataset/optdigits.csv", header=F)
opt.X <- optdigits[,1:64]
opt.y <- optdigits[,65]

opt.tsne <- as.data.frame(Rtsne(X = opt.X)$Y)

ggplot(opt.tsne, aes(x=V1, y=V2)) + geom_point()
```

Donde podemos observar que los elementos se distribuyen, de forma
más o menos ordenada, en grupos de puntos convexos. Se distinguen,
a priori, 10 u 11 grupos de puntos. Como sabemos que los puntos
vienen de dígitos manuscritos, vamos a tratar de agruparlos en 10
clusters distintos, para ver si los grupos que hemos visto en la
gráfica anterior corresponden con las clases reales de los puntos:

```{r, fig.height=3.6}
kmeans.opt <- kmeans(opt.X, 10)
opt.tsne$pred.class <- as.factor(kmeans.opt$cluster)

ggplot(opt.tsne, aes(x=V1, y=V2, col=pred.class)) + geom_point()
```

Podemos observar que, aunque los grupos no están perfectamente
separados, sí que hay varios de ellos que se han reconocido
correctamente (los grupos que se encuentran en el exterior del
conjunto están mejor delimitados que los del centro). De hecho,
si ahora tratamos de ver la matriz de confusión, tenemos lo
siguiente

```{r}
kable(table(opt.y, opt.tsne$pred.class))
```

Podemos observar que en el primer cluster están la mayoría de dígitos
1, en el tercer cluster están casi todos los 0 (es el grupo mejor
definido, con menos errores de ambos tipos), en el 4 cluster están los
dígitos 2, o en el cluster 6, que están los dígitos 6. Los grupos 9 y
10 son los que peor definidos están, el 9 por tener muchos elementos
en varios grupos (tiene a la mayoría de dígitos 3, pero también tiene
un importante número de 9), y el 10 por tener pocos elementos (tiene
en su mayoría dígitos 1, pero hay muchos más de estos dígitos en el
primer cluster). Este es un ejemplo de conjunto de datos en el que
los algoritmos de agrupamiento se comportan correctamente.

Otra opción posible es hacer el clustering directamente sobre los
grupos obtenidos por TSNE. Al igual que se hace cuando se utiliza PCA,
se puede hacer una selección de características con TSNE y aplicar el
algoritmo de agrupamiento sobre los datos reducidos. El resultado
que obtenemos en ese caso es el siguiente:

```{r}
kmeans.opt <- kmeans(opt.tsne[,1:2], 10)
opt.tsne$pred.class <- as.factor(kmeans.opt$cluster)

ggplot(opt.tsne, aes(x=V1, y=V2, col=pred.class)) + geom_point()
```

Ahora sí que tenemos un agrupamiento que a simple vista parece de
bastante calidad. Los puntos parecen estar bien colocados dentro de
sus grupos, y si mostramos la tabla que mostramos anteriormente:

```{r}
kable(table(opt.y, opt.tsne$pred.class))
```

Tenemos un agrupamiento mucho más correcto que el que encontramos al
utilizar todas las variables. Por poner algunos ejemplos, en el
cluster 7 están los dígitos 0, y sólamente hay dos elementos mal
clasificados. En el cluster 10 están los dígitos 3, y hay 11 dígitos 3
mal colocados en otros clusters, así como 10 ejemplos dentro de este
grupo que representan otro dígito. Por otro lado, el dígito 1 no se
ha colocado claramente en ningún cluster, teniendo más o menos la mitad
de sus ejemplos en el cluster 3, pero con un número importante también
en el cluster 8, el cual comparte con el 9.

Es importante remarcar que no hemos tenido en cuenta la clase en
ningún momento excepto a la hora de configurar las tablas. Tanto la
reducción de dimensionalidad utilizando TSNE como el agrupamiento han
sido realizados de manera no supervisada, aunque luego hayamos
contrastado que los agrupamientos que hemos obtenido sí que hacen
referencia a las clases definidas a priori sobre el problema.

Una vez que hemos hecho los agrupamientos, podemos estudiar los valores
de las variables que tenemos, para tratar de obtener información de los
grupos que hemos encontrado.

Por ejemplo, vamos a estudiar detenidamente el grupo 7, que hemos
visto que tiene la mayoría de dígitos 0. Si nos restringimos a los
puntos que componen dicho cluster, calculamos la media de las 64
variables, las reordenamos en una matriz $8 \times 8$, y mostramos
esa matriz como un mapa de calor, obtenemos lo siguiente:

```{r}
means <- apply(opt.X[opt.tsne$pred.class == 6,], 2, mean)
means.mat <- matrix(means, nrow=8, byrow=T)
heatmap.2(means.mat, dendrogram = "none", Rowv = FALSE, Colv=FALSE, trace='none')
```

Tenemos que los colores más cercanos al rojo representan medias más
bajas, y colores más cercanos al amarillo y al blanco representan
medias más altas. Como era de esperar, dada la reorganización que
hemos hecho, obtenemos algo similar a la silueta de un 0. En las
casillas centrales de arriba y abajo (correspondientes a las variables
4, 5, 60 y 61) tenemos una media de píxeles negros muy alta, mientras
que en los bordes derecho e izquierdo, así como en los cuadros
centrales, la media de píxeles coloreados es muy baja.

Este es un ejemplo de cómo interpretar la información proveniente de
un algoritmo de agrupamiento no supervisado, aunque se ha utilizado
cierta información del conjunto de datos y sus etiquetas para guiar la
búsqueda y mostrar información relevante.
