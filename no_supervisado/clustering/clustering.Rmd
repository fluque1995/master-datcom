...
```{r}
dataset.path <- "dataset/Absenteeism_at_work.csv"

read.dataset <- function(dataset.path){
    dataset <- read.csv(dataset.path, sep = ";")

    ## Eliminamos columnas que no aportan información
    dataset$ID <- NULL
    dataset$Body.mass.index <- NULL # Este valor es función del peso y la altura
    dataset$Seasons <- NULL # Este valor codifica la misma información que el mes
    dataset$Reason.for.absence <- NULL # A priori no contemplaremos este valor
