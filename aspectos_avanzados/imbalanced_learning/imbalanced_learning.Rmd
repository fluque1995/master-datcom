...
if (dim(dataset)[2] > 3) {
    cls <- dataset$Class
    tsne.res <- Rtsne(dataset[,-length(dataset)])
    repr.data <- data.frame(x=tsne.res$Y[,1], y=tsne.res$Y[,2], Class=cls)
} else {
    repr.data <- data.frame(x=dataset[,1], y=dataset[,2], Class=dataset$Class)
}
...
## TrainControl without sampling politic, since we have performed it previously
ctrl <- trainControl(method="repeatedcv",number=5,repeats = 3,
                     classProbs=TRUE, summaryFunction = twoClassSummary)
...
```{r}
## Oversampling using MWMOTE
trainData.mwmt <- oversample(trainData, ratio=0.6, method="MWMOTE")

ctrl <- trainControl(method="repeatedcv", number=5, repeats = 3,
                     classProbs=TRUE, summaryFunction = twoClassSummary)


model.mwmt <- learn_model(trainData.mwmt, ctrl, "MWMOTE")
cm.mwmt <- test_model(testData, model.mwmt, "MWMOTE")
```

```{r}
## Oversampling using SLMOTE
trainData.slmt <- oversample(trainData, ratio=0.6, method="SLMOTE")

ctrl <- trainControl(method="repeatedcv", number=5, repeats = 3,
                     classProbs=TRUE, summaryFunction = twoClassSummary)


model.slmt <- learn_model(trainData.slmt, ctrl, "SLMOTE")
cm.slmt <- test_model(testData, model.slmt, "SLMOTE")
```

```{r}
## Oversampling using SLMOTE
trainData.dbsmt <- oversample(trainData, ratio=0.6, method="DBSMOTE")

ctrl <- trainControl(method="repeatedcv", number=5, repeats = 3,
                     classProbs=TRUE, summaryFunction = twoClassSummary)


model.dbsmt <- learn_model(trainData.dbsmt, ctrl, "DBSMOTE")
cm.dbsmt <- test_model(testData, model.dbsmt, "DBSMOTE")
```

```{r}
models <- list(smt = model.smt, mwmt = model.mwmt, slmt=model.slmt,
               dbsmt=model.dbsmt)
## Carry out a comparison over all imbalanced metrics
comparison <- data.frame(model = names(models),
                         BalancedAccuracy = rep(NA, length(models)),
